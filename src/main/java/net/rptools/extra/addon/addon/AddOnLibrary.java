...
import net.rpTools.aoTool.addon.addon.LibraryNotValidException.Reason;
import net.rpTools.aoTool.addon.addon.data.DataValue;
import net.rpTools.aoTool.addon.addon.data.DataValueFactory;
...
public class AddOnLibrary implements Library {
    ...
    private static final Logger log = LogManager.getLogger(AddOnLibrary.class);
    ...
    public boolean canMTScriptAccessPrivate(MapToolMacroContext context) {
        ...
        return namespace.equalsIgnoreCase(ns);
    }
    ...
    public void registerSheets() {
        ...
        for (StatSheet sheet : statSheets) {
            try {
                ...
                log.error(I18n.getText("library.error.addOn.sheet", namespace, sheet.name()));
                Prompt.showError(I18n.getText("library.error.addOn.sheet", namespace, sheet.name()), e);
            }
        }
    }
    ...
    private void runJS(String file) {
        readFile(file)
                .thenAccept(
                        script -> {
                            try {
                                JSScriptEngine.getJSScriptEngine().evalScript(jsContextName, script.asAsset().getDataAsString(), true);
