...
public record StatSheet(
  ...
  private static final Function<JsonObject, URL> toURL = jso -> {
    try {
      String address =
              "lib://" +
              (!jso.keySet().contains("namespace")?"namespace":jso.get("namespace").getAsString())
              + "/" +
              (!jso.keySet().contains("entry")?"":jso.get("entry").getAsString());
      return new URI(address).toURL();
    } catch (Exception ignored){
    }
    return null;
  };
  public StatSheet(JsonObject jso) throws MalformedURLException {
    this(
            !jso.keySet().contains("name") ? null : jso.get("name").getAsString(),
            !jso.keySet().contains("description") ? null : jso.get("description").getAsString(),
            !jso.keySet().contains("entry") ? null : toURL.apply(jso),
            !jso.keySet().contains("name") ? null : jso.get("propertyTypes").getAsJsonArray().asList().stream().map(JsonElement::getAsString).collect(Collectors.toSet()),
            !jso.keySet().contains("namespace") ? null : jso.get("namespace").getAsString()
    );
  }
public StatSheet(){
