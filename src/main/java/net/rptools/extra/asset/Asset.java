...
import net.rpTools.aoTool.files.FileType;
        ...
        public static Type fromMediaType(MediaType mediaType, String filename) {
            ...
            return switch (contentType) {
                ...
                case "application" -> switch (subType) {
                  ...
                  if (AddOnLibraryImporter.isAssetFileAddOn(filename)) {
                            yield Type.MTLIB;
                  }
                        }
                        yield Type.INVALID;
                ...
                case "model" -> subType.equals("vnd.mts") ? Type.TEXT : Type.INVALID;
                default -> Type.INVALID;
            };
        }
    }
...
//    @XStreamConverter(AssetImageConverter.class)
    ...
    /**
     * Create an {@code Asset} for an invalid type. The asset won't actually be created and saved only
    ...
    public static Asset createAssetDetectType(String name, byte[] data) throws IOException {
        MediaType mediaType = FileType.getMediaType(name, data);
...
//    }
    ...
    private String determineImageExtension() {
        ...
        try {
            ...
            log.info("IOException?!", e); // Can this happen??
        }
        return ext;
    }
     ...
     * @throws IllegalStateException if this {@code Asset} cannot be represented as a {@link JsonElement}.
     */
    ...
    public MediaType getMediaType() {
        try {
            return FileType.getMediaType(this.name, this.data);
        } catch (IOException ignored) {
            return switch (this.type) {
                case IMAGE -> MediaType.create(MediaType.ANY_IMAGE_TYPE.type(), this.extension);
                case AUDIO -> MediaType.create(MediaType.ANY_AUDIO_TYPE.type(), this.extension);
                case CSS -> MediaType.CSS_UTF_8;
                case HTML -> MediaType.HTML_UTF_8;
                case JSON -> MediaType.JSON_UTF_8;
                case JAVASCRIPT -> MediaType.JAVASCRIPT_UTF_8;
                case MTLIB -> MediaType.ZIP;
                case MARKDOWN -> MediaType.MD_UTF_8;
                case TEXT -> MediaType.PLAIN_TEXT_UTF_8;
                case XML -> MediaType.APPLICATION_XML_UTF_8;
                default -> MediaType.PLAIN_TEXT_UTF_8;
            };
        }
    }
    ...
    public static Type getTypeFromFileExtension(String extension) {
        return FileType.mediaTypeToAssetType(getMediaTypeFromFileExtension(extension));
    }
    public static Type getType(String filename, InputStream is) throws IOException {
        return FileType.mediaTypeToAssetType(getMediaType(filename, is));
    }
    public static Type getType(String filename, byte[] bytes) throws IOException {
        return FileType.mediaTypeToAssetType(getMediaType(filename, bytes));
    }
    public static Type getType(URL url) throws IOException {
        return FileType.mediaTypeToAssetType(getMediaType(url));
    }
    public static Type getType(MediaType type) { return FileType.mediaTypeToAssetType(type); }
    public static MediaType getMediaTypeFromFileExtension(String extension) {
        return FileType.getMediaTypeFromFileExtension(extension);
    }
    public static MediaType getMediaType(String filename, InputStream is) throws IOException {
        return FileType.getMediaType(filename, is);
    }
    public static MediaType getMediaType(String filename, byte[] bytes) throws IOException {
        return FileType.getMediaType(filename, bytes);
    }
    public static MediaType getMediaType(URL url) throws IOException {
        return FileType.getMediaType(url);
    }
    public static MediaType getMediaType(Type type) { return FileType.assetTypeToMediaType(type);}
}